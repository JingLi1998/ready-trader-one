...
def __on_task_completed(
    future: concurrent.futures.Future, name: str, executor: concurrent.futures.Executor
) -> None:
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor(
        max_workers=len(trader_names) + 1
    ) as executor:
        exchange = executor.submit(ready_trader_one.exchange.main)
        exchange.add_done_callback(
            functools.partial(__on_task_completed, name="exchange", executor=executor)
        )
        ...
        traders = [
            executor.submit(ready_trader_one.trader.main, name) for name in trader_names
        ]
        for name, task in zip(trader_names, traders):
            task.add_done_callback(
                functools.partial(__on_task_completed, name=name, executor=executor)
            )
